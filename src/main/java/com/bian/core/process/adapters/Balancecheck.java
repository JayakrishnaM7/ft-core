...
@Component
public class Balancecheck implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private BalancecheckRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Balancecheck.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Balancecheck.class.getName());
         }else{
