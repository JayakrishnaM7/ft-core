...
@Component
public class Benificiaries implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private BeneficiariesRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Benificiaries.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Benificiaries.class.getName());
         }else{
