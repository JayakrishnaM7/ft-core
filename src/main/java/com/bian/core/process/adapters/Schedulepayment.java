...
@Component
public class Schedulepayment implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private SchedulepaymentRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Schedulepayment.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Schedulepayment.class.getName());
         }else{
