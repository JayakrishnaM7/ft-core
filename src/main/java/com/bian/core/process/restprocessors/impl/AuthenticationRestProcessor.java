...
@Service
public class AuthenticationRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(AuthenticationRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.authentication.hosturl}")
	private String url;
	
	@Value("${spring.api.authentication.context-path}")
	private String contextPath;

	@Value("${spring.api.authentication.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  AuthenticationRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ AuthenticationRestProcessor.class.getName());
