...
@Service
public class BalancecheckRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(BalancecheckRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.balancecheck.hosturl}")
	private String url;
	
	@Value("${spring.api.balancecheck.context-path}")
	private String contextPath;

	@Value("${spring.api.balancecheck.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  BalancecheckRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ BalancecheckRestProcessor.class.getName());
