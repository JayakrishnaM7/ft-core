...
@Service
public class NotificationRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(NotificationRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.notification.hosturl}")
	private String url;
	
	@Value("${spring.api.notification.context-path}")
	private String contextPath;

	@Value("${spring.api.notification.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  NotificationRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ NotificationRestProcessor.class.getName());
