package com.bian.core.process.restprocessors.impl;

import com.bian.core.process.restprocessors.RestApiInterface;
...
@Service
public class SchedulepaymentRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(SchedulepaymentRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.schedulepayment.hosturl}")
	private String url;
	
	@Value("${spring.api.schedulepayment.context-path}")
	private String contextPath;

	@Value("${spring.api.schedulepayment.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  SchedulepaymentRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ SchedulepaymentRestProcessor.class.getName());
